...
radiologia.shift() //Eliminando el primer elemento
radiologia.pop() //Elimnando último elemento 

console.table(radiologia); //Arreglo modificado de Radiología
...
document.write(`<h3>LISTA DENTAL:</h3>`)
...
document.write(`<h3>PACIENTES ATENDIDOS EN EL CENTRO:</h3>`);

todosLosPacientes = [];
let pacientesTotales = todosLosPacientes.concat(radiologia, traumatologia, dental);

for (let i = 0; i < pacientesTotales.length; i++){
document.write(`<p>${pacientesTotales[i].paciente}</p>`);
}
...
console.table(previsionTraumatologia); // Se añaden los pacientes de la tabla agregada en el requerimiento 1
